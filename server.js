...
io.on("connection", socket => {
  socket.on("CODE_CHANGED", async code => {
    ...
    let length_username = username.length;

    // io.emit('CODE_CHANGED', code)
    if (username.charAt(length_username - 2) == "/") {
      if (username.charAt(length_username - 1) == "O") {
        socket.to(roomName).emit("CODE_CHANGED", code);
      }
      // socket.to(roomName).e
    } else {
      let length_room = roomId.length;
      if (roomId.charAt(length_room - 2) == "/") {
        if (roomId.charAt(length_room - 1) == "E") {
          socket.to(roomName).emit("CODE_CHANGED", code);
        }
        // socket.to(roomName).e
      }
